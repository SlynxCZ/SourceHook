...
tl::expected<InlineHook, InlineHook::Error> InlineHook::create(void* target, void* destination, InlineHook::Flags flags) {
    return create(Allocator::global(), target, destination, flags);
}

tl::expected<InlineHook, InlineHook::Error> InlineHook::create(
    const std::shared_ptr<Allocator>& allocator, void* target, void* destination, InlineHook::Flags flags) {
    InlineHook hook{};
...
tl::expected<MidHook, MidHook::Error> MidHook::create(void* target, MidHookFn destination, MidHook::Flags flags) {
    return create(Allocator::global(), target, destination, flags);
}

tl::expected<MidHook, MidHook::Error> MidHook::create(
    const std::shared_ptr<Allocator>& allocator, void* target, MidHookFn destination, MidHook::Flags flags) {
    MidHook hook{};
...
tl::expected<void, MidHook::Error> MidHook::setup(
    const std::shared_ptr<Allocator>& allocator, uint8_t* target, safetyhook::MidHookFn destination_fn) {
    m_target = target;
